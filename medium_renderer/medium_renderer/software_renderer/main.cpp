...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE no, PWSTR args, int cmdShow)
{
	...
	renderer::Renerer renderer(window->canvas, camera.Position);
	...
	while (gui::Window::is_running(window))
	{
		...
		shader.set_projection(camera.get_projection(window->canvas.width, window->canvas.height));


		// Draw models
		renderer.RenderModel(head, shader);
		renderer.RenderModel(plane, shader);
		renderer.RenderModel(diablo, shader);
		renderer.RenderModel(cube, shader);
		renderer.RenderModel(boggie, shader);
		...
		light_shader.Transforms = camera.get_projection(window->canvas.width, window->canvas.height) * camera.get_lookat() * model;

		// Draw lighter
		renderer.RenderMesh(*lighter.mesh, light_shader);
		...
		// Draw lighter
		renderer.RenderMesh(*lighter2.mesh, light_shader);
